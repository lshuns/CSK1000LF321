# -*- coding: utf-8 -*-
# @Author: lshuns
# @Date:   2023-04-11 17:43:27
# @Last Modified by:   lshuns
# @Last Modified time: 2023-04-22 22:02:35
...
in_file_fiducial = '/disks/shear10/ssli/K1000CS/skills_v07D7p1_Outputs/skills_v07D7p1_LF_321_kidsPhotometry_shear_noSG_newCut_A1_WeiCut_goldclasses.cat.A2.feather'

##### the PSF modelling cata (without gold selection)
...
# ##### the PSF modelling cata (with gold selection)
# in_file_test = '/disks/shear16/ssli/ImSim/output/skills_v07D7p1_PSFmodelling/skills_v07D7p1_LF_321_combined_PSFmodelling41_321_noSG_noWeiCut_newCut_goldSelected.feather'
# ## where to save results
# outpath = './results/dm_PSFmodelling_41_rewei_gold.csv'
...
def dmCalFunc_tile_based(cataSim0, cataSim1, nboot=500):
    ...
    if (not np.array_equal(cataSim0['tile_label'].values, cataSim1['tile_label'].values)) \
        | (not np.array_equal(np.array(cataSim0['g1_in'].values*10000, dtype=int), 
            np.array(cataSim1['g1_in'].values*10000, dtype=int))) \
        | (not np.array_equal(np.array(cataSim0['g2_in'].values*10000, dtype=int), 
            np.array(cataSim1['g2_in'].values*10000, dtype=int))):
